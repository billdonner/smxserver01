...
extension ReportMaker {
    ...
    ///
    class  func report_from_bunchoffollowers(igp:SocialDataProcessor, bop:BunchOfPeople,skip:Int = 0,limit:Int = 1000)-> ReportResult {
    ...
    ///
    class  func all_followers_report(igp:SocialDataProcessor,skip:Int=0,limit:Int=1000) -> ReportResult  { //TODO
        let newfollowers = igp.pd.ouAllFollowers
        //        newfollowers.sort {
        //            $0.
        //        }
        // TODO: sort as in ios app
        return ReportMaker.report_from_bunchoffollowers(igp:igp, bop:newfollowers,skip:skip,limit:limit)
    }
    ///
    // MARK:- Booster Followers with Highest Avg Like Rate
    ///
    class  func booster_followers_report(igp:SocialDataProcessor,skip:Int=0,limit:Int=1000) -> ReportResult  { //TODO
    ...
    ///
    class  func ghost_followers_report(igp:SocialDataProcessor,skip:Int=0,limit:Int=1000) -> ReportResult  {
        var newfollowers:BunchOfPeople = []
        for follower in igp.pd.ouAllFollowers {
            if igp.likersDict [follower.id] == nil {
                newfollowers.append(follower)
    ...
    ///
    class func makefollowingdict(people:BunchOfPeople)->PeopleDict {
        var d: PeopleDict = PeopleDict()
        for person in people { d[person.id] = person }
        return d
    }
    class func unrequited_followers_report(igp:SocialDataProcessor,skip:Int=0,limit:Int=1000) -> ReportResult  {
        
        let fd = makefollowingdict(people:igp.pd.ouAllFollowing)
        var newfollowers:BunchOfPeople = []
        for follower in igp.pd.ouAllFollowers {
    ...
    ///
    class  func secret_admirer_followers_report(igp:SocialDataProcessor,skip:Int=0,limit:Int=1000) -> ReportResult  { //TODO
        var newfollowers:BunchOfPeople = []
        for follower in igp.pd.ouAllFollowers {
            if igp.likersDict [follower.id] == nil {
                newfollowers.append(follower)
